...
Para un despliegue exitoso, asegúrese de configurar las siguientes variables en su entorno de producción (ej. Docker, Heroku, AWS, etc.) o en un archivo `.env` ubicado en la raíz del backend (`backend/.env`).
...
| `ENVIRONMENT` | Entorno de ejecución | Establecer a `production` para desactivar la documentación automática de la API (`/docs`). |
